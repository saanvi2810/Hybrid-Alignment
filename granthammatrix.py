...
#Biopython grantham distance matrix
grantham_data = {
...
df = pd.DataFrame.from_dict(grantham_data, orient='index', columns=AMINO_ACIDS)
df = df.reindex(index=AMINO_ACIDS, columns=AMINO_ACIDS)
grantham_matrix = df.astype(float)
normalized_grantham_matrix = 1 - (grantham_matrix / grantham_matrix.values.max())

grantham_matrix, normalized_grantham_matrix
